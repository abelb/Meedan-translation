...
$(document).ready( function() {
                ...
                success: function(msg){
                        ...
                        for (i in data) {
                                if (data[i].length > 0) {
                                        ...
                                        if (existing.indexOf('ICanLocalize') < 0) {
                                                existing += '<br /><b>ICanLocalize</b>';
                                        }
                                        existing += '<br />(' + string[1] + ' - ' + string[2] + ') ';
        ...
        if (l10n_client.length) {
                ...
                $('#l10n-client-form .form-textarea').css('height', '11em');
                ...
                $('#edit-clear').after(span);
                span.hide();
                
                clone = $('#l10n-client-form #edit-copy').clone();
                clone.attr('id', 'icl-send-translation');
                clone.attr('value', Drupal.settings.send_button);
                
                $('#edit-clear').after(clone);
                ...
                span = $(document.createElement('span'))
                span.html('<b>ICanLocalize: </b>');
                $('#edit-clear').after(span);

                $('#edit-clear').after($(document.createElement('br')));
                $('#edit-clear').after($(document.createElement('br')));
                ...
                $('#l10n-client #icl-send-translation').click(function() {
                        ...
                        data: "string=" + $('#l10n-client-string-editor .source-text').text(),
                        async: true,
                        success: function(msg){
                                data = msg.split('|');
                                if (data[0] == '1') {
